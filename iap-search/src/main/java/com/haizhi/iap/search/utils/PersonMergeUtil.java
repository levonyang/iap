...
public class PersonMergeUtil
{
     ...
     * @return GraphVo 融合自然人之后的图数据
     */
    public GraphVo mergePerson(GraphVo graph)
    {
        ...
        graph.getVertexes().stream().filter(vertex -> {
            return !CollectionUtils.isEmpty(vertex) && VertexCollection.PERSON.equals(StringUtils.substringBefore(vertex.get("_id").toString(), "/"));
        ...
        return new GraphVo(otherVertices, graph.getEdges());
    }
     ...
     * @return
     */
    public List<Map<String, Map<String, Object>>> expandWithPersonMerge(final String withDocuments, Collection<String> edgeTables, final int depth, Set<String> startVertices, GraphEdge.Direction direction, String filterStr, Map<String, Object> params)
    {
        ...
        int i = 0;
        Set<String> personIds = startVertices.stream().filter(vertex -> VertexCollection.PERSON.equals(StringUtils.substringBefore(vertex, "/"))).collect(Collectors.toSet());
        ...
        {
            List<Map<String, Map<String, Object>>> result = graphRepo.traversalGraphWithParams(withDocuments, edgeTables, 1, startVertices, direction, filterStr, params, null, null);
            ...
            personIds = startVertices.stream().filter(vertexId -> VertexCollection.PERSON.equals(StringUtils.substringBefore(vertexId, "/"))).collect(Collectors.toSet());
            if(!CollectionUtils.isEmpty(personIds))
            {
                List<Map<String, Object>> mergedPersonEdges = findMergedPersonEdges(personIds);
                personIds = mergedPersonEdges.stream().flatMap(edge -> Stream.of(edge.get("_from").toString(), edge.get("_to").toString()).distinct()).collect(Collectors.toSet());
    ...
    /**
     * 查询时，按person_merge展开，并限定返回的结果集范围，
     * params 中必须包含参数vertices
     * @param withDocuments
     * @param edgeTables
     * @param depth
     * @param startVertices
     ...
     * @return
     */
    public List<Map<String, Map<String, Object>>> expandWithPersonMergeRangedResult(final String withDocuments, Collection<String> edgeTables, final int depth, Set<String> startVertices, GraphEdge.Direction direction, String filterStr, Map<String, Object> params) {
        if (CollectionUtils.isEmpty(edgeTables)) return Collections.emptyList();
        List<Map<String, Map<String, Object>>> results = new ArrayList<>();
        ...
        while (!CollectionUtils.isEmpty(startVertices) && i++ < depth) {
            List<Map<String, Map<String, Object>>> result = graphRepo.traversalGraphWithPersonMerge(withDocuments, edgeTables, 1, startVertices, direction, filterStr, params, null, null);
            if (!CollectionUtils.isEmpty(result)) results.addAll(result);
